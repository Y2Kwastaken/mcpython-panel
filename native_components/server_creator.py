...
def onCall(panel: Panel_Interface):
    ...
    with open(server_path+"/server.properties", "a") as file:
        port = cinput("&3Port >> ")     
        allow_nether = bool(cinput("&3Allow Nether [true/false]>>"))
        max_players = int(cinput("&3Max Players >> "))
        view_distance = int(cinput("&3View Distance >> "))
    ...
    with open(server_path+"/spigot.yml", "a") as file:
        bungeecord = bool(cinput("&3Bungee [true/false] >> "))
